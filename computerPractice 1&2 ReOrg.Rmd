...
###### SPX (data pre loaded, ret pre caled,see 1.1)
...
for (i in ((m_days+1):Nspx)){
  underlying[i,'var']=1/(m_days-1)*sum((
...
underlying <- 
  underlying %>% 
  mutate(stdAnnualSPX=(var*252/m_days)^0.5)
...
spotUndelryingSPX=underlying[underlying$TradeDate==selectedTradeDate,'SPX']
volatilitySPX=underlying[underlying$TradeDate==selectedTradeDate,'stdAnnualSPX']

spotUndelrying2 <- 
  underlying %>%
  filter(TradeDate==selectedTradeDate) %>%  
  select(SPX,TradeDate) 

volatilitySPX2 <- 
  underlying %>% 
  filter(TradeDate==selectedTradeDate) %>% 
  select(stdAnnualSPX,TradeDate)
...
underlying <- 
  underlying %>% 
  mutate(stdAnnualSPY=(var*252/m_days)^0.5)
...
spotUndelryingSPY=underlying[underlying$TradeDate==selectedTradeDate,'SPY']
volatilitySPY=underlying[underlying$TradeDate==selectedTradeDate,'stdAnnualSPY']

spotUndelryingSPY2 <- 
  underlying %>%
  filter(TradeDate==selectedTradeDate) %>%  
  select(SPY,TradeDate) 

volatilitySPY2 <- 
  underlying %>% 
  filter(TradeDate==selectedTradeDate) %>% 
  select(stdAnnualSPY,TradeDate)
