...
spyOptionCross=read.csv("./sampleData/spyOptionData20200110.csv", #check.names=FALSE,
               colClasses=c("numeric","character","character",rep("numeric",3)))
...
oneExpiryOptionDataspx <- 
  spxOptionCross %>% 
  filter(expiryDate==selectedExpiry)

#spyOptionCross %<>%   arrange(expiryDate)

sort(unique(spyOptionCross$expiryDate))

selectedExpiry <- "2020-06-19"#select from sort?

oneExpiryOptionDataspy <- 
  spyOptionCross %>% 
  filter(expiryDate==selectedExpiry)
...
###before running the loop, using fake number to assign values first.
```{r 3.1.1 EUR/AMRI-CALL load in parameter}
...
```{r 3.1.2 EUR/AMRI-CALL load parameters}
nSteps=1000
interestRate=0.05
dividendYield=0.02
volatility=0.2 
